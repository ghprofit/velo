...
} from '../services/veriffService.js';

/**
 * Initiate Veriff verification session for creator
 */
export async function initiateVerification(req: Request, res: Response): Promise<void> {
  try {
    if (!req.user) {
      res.status(401).json({
        success: false,
 ...
 */
export async function handleVeriffWebhook(req: Request, res: Response): Promise<void> {
  try {
    ...
    const event: VeriffWebhookEvent = req.body;
 ...
 */
export async function manualVerification(req: Request, res: Response): Promise<void> {
  try {
